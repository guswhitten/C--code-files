...
public:
    bool canFinish(int n, vector<vector<int>>& p) {
        ...
        if(p.empty()) return true; //all courses independent
        ...
        if(q.empty()) return false;  //cycle exists if there is no node with indegree 0
        int taken = 0; //No. of nodes you're able to reach till the end
        while(!q.empty()){
            int curr = q.front(); 
            q.pop();
            taken++;
